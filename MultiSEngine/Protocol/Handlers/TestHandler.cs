            ...
            switch (msgType)
            {
                case MessageID.Kick:
                    if (context.Packet is not Kick kick)
                        throw new Exception("[TestHandler] Kick packet not found");
                ...
                case MessageID.LoadPlayer:
                    if (context.Packet is not LoadPlayer slot)
                        throw new Exception("[TestHandler] LoadPlayer packet not found");
                    ...
                    await SendToServerDirectAsync(new SyncPlayer
                    {
                        ...
                        Bit2 = new BitsByte(),
                        HideMisc = 0,
                        HairColor = Utils.Rgb(0x00, 0x00, 0xFF),
                        SkinColor = Utils.Rgb(0x00, 0x00, 0xFF),
                        EyeColor = Utils.Rgb(0x00, 0x00, 0xFF),
                        ShirtColor = Utils.Rgb(0x00, 0x00, 0xFF),
                        UnderShirtColor = Utils.Rgb(0x00, 0x00, 0xFF),
                        PantsColor = Utils.Rgb(0x00, 0x00, 0xFF),
                        ShoeColor = Utils.Rgb(0x00, 0x00, 0xFF),
                        Bit3 = new BitsByte(),
                        Bit4 = new BitsByte(),
                        Bit5 = new BitsByte()
                ...
                case MessageID.WorldData:
                    if (!TestParent.IsSuccess.HasValue)
                    {
                        ...
                        {
                            PlayerSlot = index,
                            Position = new Terraria.DataStructures.Point16(-1, -1),
                            Timer = 0,
                            DeathsPVE = 0,
                            DeathsPVP = 0,
