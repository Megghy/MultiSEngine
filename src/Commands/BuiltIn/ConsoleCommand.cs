                ...
                case "test":
                    if (parma.Any())
                    {
                        ...
                        if (parma[0].ToLower() == "all")
                            await Networking.ProxyServer.TestAllAsync(showDetail).ConfigureAwait(false);
                        else if (Utils.GetSingleServerInfoByName(parma[0]) is { } testServer)
                            await Networking.ProxyServer.TestConnectAsync(testServer, showDetail).ConfigureAwait(false);
                        else
