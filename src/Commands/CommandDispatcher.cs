...
namespace MultiSEngine.Commands
{
        ...
        public static async ValueTask<(bool handled, bool continueSend)> HandleCommand(ClientData client, string text, bool fromConsole = false)
        {
            ...
            if (text?.StartsWith("/") ?? false)
            {
                ...
                if (num != 0)
                {
                    ...
                    if (fromConsole)
                        aviliableCommands = RuntimeState.Commands.FindAll(c => c.ServerCommand && (c.Name.ToLower() == cmdName.ToLower() || cmdName.Contains(c.Name)));
                    else
                        aviliableCommands = RuntimeState.Commands.FindAll(c => c.Name.ToLower() == cmdName.ToLower() && !c.ServerCommand);
